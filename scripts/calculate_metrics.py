...
def network_metrics(tables, attackers):
    ...
    RX is de-duplicated on (sender, seq) first: a datagram delivered twice
    after a link-layer retransmission must not make the ratio exceed 1.

    Control-plane overhead is DIO/DIS *receptions* per monitoring node per
    minute. It is counted from the per-message EV records over the whole run,
    which is the only span the two detector modes share: the paper mode stops
    evaluating at the last completed 5-minute window (1500 s of an 1800 s run)
    while the sliding mode evaluates to 1790 s, so normalising either mode by
    its own last NBR timestamp compares different fractions of the same attack.
    When EV logging is off the NBR counters are used instead, corrected by
    IDS_SLIDE_BUCKETS in sliding mode (each message is counted in that many
    consecutive evaluations); that path is approximate and is flagged in
    `overhead_source`.
    """
    tx, rx, nbr, par, block, stat, ids, atk, ev = (tables[k] for k in
        ("TX", "RX", "NBR", "PAR", "BLOCK", "STAT", "IDS", "ATK", "EV"))
    out = {}
    normals = set(stat.mote.unique()) - set(attackers)
    # Packet delivery ratio, all senders and normal senders only.
    tx_n = len(tx)
    rx_u = rx.drop_duplicates(subset=["from", "seq"]) if len(rx) else rx
    rx_n = len(rx_u)
    out["tx_total"], out["rx_total"] = int(tx_n), int(rx_n)
    out["rx_duplicates"] = int(len(rx) - rx_n)
    out["pdr"] = (rx_n / tx_n) if tx_n else float("nan")
    tx_norm = tx[tx.mote.isin(normals)]
    rx_norm = rx_u[rx_u["from"].isin(normals)] if len(rx_u) else rx_u
    ...
    # Control-plane overhead: receptions per monitoring node per minute.
    mode = str(ids.iloc[0]["mode"]) if len(ids) else "paper"
    n_mon = nbr.mote.nunique() if len(nbr) else 0
    run_end = max((float(d.t.max()) for d in tables.values() if len(d)), default=0.0)
    if len(ev) and n_mon and run_end:
        out["overhead_source"] = "ev"
        span_min = run_end / 60.0
        out["dio_rx_per_node_min"] = float((ev.kind == "DIO").sum()) / n_mon / span_min
        out["dis_rx_per_node_min"] = float((ev.kind == "DIS").sum()) / n_mon / span_min
    elif n_mon and len(nbr):
        # Fallback: no per-message log. Approximate from the window counters.
        out["overhead_source"] = "nbr"
        factor = 6.0 if mode == "sliding" else 1.0
        span_min = float(nbr.t.max()) / 60.0
        out["dio_rx_per_node_min"] = float(nbr.dio.sum()) / factor / n_mon / span_min
        out["dis_rx_per_node_min"] = float(nbr.dis.sum()) / factor / n_mon / span_min
    else:
        out["overhead_source"] = "none"
        out["dio_rx_per_node_min"] = out["dis_rx_per_node_min"] = float("nan")
    # Routing churn. Every non-root RPL node reports a parent switch (the
    # RPL_CALLBACK_PARENT_SWITCH hook is compiled into the attacker firmwares
    # too), but only normal nodes emit STAT, so the denominator must be the
    # union of both populations or attackers inflate the ratio.
    rpl_nodes = set(stat.mote.unique()) | set(par.mote.unique())
    out["parent_changes_per_node"] = (len(par) / len(rpl_nodes)) if rpl_nodes else float("nan")
    ...
    # Blocks that landed on honest nodes: the operational cost of a false positive.
    if len(block):
        bn = block[block.nbr.isin(normals) & block.action.isin(["temp", "perm"])]
        out["blocks_temp_normal"] = int((bn.action == "temp").sum())
        out["blocks_perm_normal"] = int((bn.action == "perm").sum())
        out["normals_blocked"] = int(bn.nbr.nunique())
    else:
        out["blocks_temp_normal"] = out["blocks_perm_normal"] = out["normals_blocked"] = 0
...
def compute(tables, truth, window_sec):
    ...
    node_flagged_any = set()    # honest nodes flagged by the attack's own rule
    node_flagged_anyrule = set()  # honest nodes flagged by ANY rule (deployed view)
    nodes_heard = set()
    first_tp_t = None
    for _, r in nbr.iterrows():
        ...
        if is_attacker and in_attack:
            ...
            if alert_kind in kinds:
                ...
                d_tn += 1
            # Deployed view: both rules run at once, so any alert on an honest
            # node is an operational false positive regardless of which rule fired.
            if kinds:
                d_fp_anyrule += 1
                node_flagged_anyrule.add(neigh)
    ...
    n_fp_any = sum(1 for n in heard_normals if n in node_flagged_anyrule)
    ...
    return {**net,
        ...
        # Deployed view: every rule runs at once, so an alert of any kind on an
        # honest node is an operational false positive.
        "node_FPR_anyrule": (n_fp_any / len(heard_normals)) if heard_normals else 0.0,
        "dec_FPR_anyrule": (d_fp_anyrule / (d_fp_anyrule + d_tn)) if (d_fp_anyrule + d_tn) else 0.0,
