...
# arm-none-eabi-gcc, Java 25 and the Cooja build dependencies, but it does
...
# Version record for the report (Phase 0 checkpoint). Runs as root, so tell
# git to ignore the ownership mismatch with the user-owned checkout.
RUN { echo "date=$(date -u +%FT%TZ)"; \
      echo "contiki_ng_tag=${CONTIKI_NG_TAG}"; \
      echo "contiki_ng_commit=$(git -c safe.directory='*' -C ${CONTIKI_NG} rev-parse HEAD)"; \
      echo "cooja_commit=$(git -c safe.directory='*' -C ${CONTIKI_NG}/tools/cooja rev-parse HEAD)"; \
...
# The base image's entrypoint (/usr/local/bin/remap-user.sh) must start as
# root: it remaps the `user` account to the host UID/GID so bind-mounted files
# keep the right owner, then drops privileges. Do NOT end with `USER user`.
USER root
